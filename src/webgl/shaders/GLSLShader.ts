...
highp vec3 fetchVec3(highp sampler2D tex, highp float componentIdx, highp vec2 invSize){
  ...
  if (modN == 0.0) {
    ...
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).xyz;
  } else if (modN == 1.0) {
    highp float t = (index + 0.5) * invSize.x;
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).yzw;
  } else if (modN == 2.0) {
    highp float t0 = (index + 0.5) * invSize.x;
    highp float x0 = fract(t0);
    highp float y0 = (floor(t0) + 0.5) * invSize.y;
...
highp vec2 fetchVec2(highp sampler2D tex, highp float componentIdx, highp vec2 invSize){
  ...
  if (modN == 0.0) {
    ...
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).xy;
  } else if (modN == 1.0) {
    highp float t = (index + 0.5) * invSize.x;
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).yz;
  } else if (modN == 2.0) {
    highp float t = (index + 0.5) * invSize.x;
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).zw;
  } else {
    highp float t0 = (index + 0.5) * invSize.x;
    highp float x0 = fract(t0);
    highp float y0 = (floor(t0) + 0.5) * invSize.y;
...
highp float fetchScalar(highp sampler2D tex, highp float componentIdx, highp vec2 invSize){
  ...
  if (modN == 0.0) {
    ...
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).x;
  } else if (modN == 1.0) {
    highp float t = (index + 0.5) * invSize.x;
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).y;
  } else if (modN == 2.0) {
    highp float t = (index + 0.5) * invSize.x;
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).z;
  } else {
    highp float t = (index + 0.5) * invSize.x;
    highp float x = fract(t);
    highp float y = (floor(t) + 0.5) * invSize.y;
    return ${this.glsl_texture}( tex, vec2(x, y) ).w;
  }
}
